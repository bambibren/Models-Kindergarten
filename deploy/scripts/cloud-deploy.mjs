...
function bootstrapCommand(remoteRelease) {
  return [
    ...
    "umask 077",
    "mkdir -p /srv/mk/data/app /srv/mk/secrets /srv/mk/backups",
    `mkdir -p ${quote(remoteRelease)}`,
    "if [ ! -f /srv/mk/secrets/mk_master_key ]; then openssl rand -base64 32 > /srv/mk/secrets/mk_master_key; fi",
    "chmod 600 /srv/mk/secrets/mk_master_key",
    "test \"$(wc -c < /srv/mk/secrets/mk_master_key)\" -eq 45",
  ].join("; ");
}
...
function remoteDeployCommand(remoteRelease, manifestName, settings) {
  ...
  return [
    ...
    "test -r /srv/mk/secrets/mk_master_key",
    "if [ -d /srv/mk/data/app ] && [ \"$(find /srv/mk/data/app -mindepth 1 -maxdepth 1 -print -quit)\" ]; then tar -C /srv/mk/data -czf /srv/mk/backups/pre-deploy-$(date +%Y%m%d%H%M%S).tgz app; fi",
    `${compose} pull`,

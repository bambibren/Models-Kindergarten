...
**Goal:** 重写主 README，让它诚实表达“上下文实验是长期核心方向，但当前可用产品主线是 HTML/PPTX 生成与 Artifact 复用”，并准确区分可管理 MCP 与内置 Exa 搜索链路。

**Architecture:** 只修改仓库根目录 `README.md`，不改变任何 Runtime、页面入口或配置。文档先解释产品愿景和当前状态，再介绍现阶段闭环、核心对象、完整 Tool 目录、MCP 特殊边界、系统结构、本地测试分支与验证方式。

**Tech Stack:** Markdown、Mermaid、现有 React Web / Remote ACP / ToolRuntime / Artifact Store。

---

### Task 1: 重写主 README 的产品定位与能力说明

**Files:**
- Modify: `README.md`

- [x] **Step 1: 用以下完整内容替换 `README.md`**
...
“模型幼儿园”最终想解决的问题，是让同一个任务在不同模型、不同Agent上下文配置下重复运行、观察和比较，帮助人理解模型为什么表现不同，以及怎样组合提示词、工具、Skills、MCP 和历史上下文才能得到更好的结果。
...
- [x] **Step 2: 检查文档格式与关键边界**

Run:

```bash
git diff --check -- README.md
rg -n "功能调研中|本地测试分支|Agent Tool 完整目录|Exa Remote MCP|Streamable HTTP|ContextAssembler" README.md
```

Expected:

- `git diff --check` 无输出并返回 0；
- `rg` 能找到上下文实验状态、线上部署定位、本地 `qwen3:8b` 测试分支、完整 Tool 目录、Exa 搜索特殊链路、页面 MCP 限制和 Resource 预加载边界。

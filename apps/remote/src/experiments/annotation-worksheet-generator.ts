...
/** 根据已校验输入构建「buildInput」结果，不额外持有调用方的大对象。 */
...
function parseRawWorksheet(text: string, experiment: ExperimentRecordV2): RawWorksheet {
  try {
    ...
    const requirements = value.requirements.map(/** 将当前元素转换为目标投影，并保持集合顺序与一一对应关系。 */
(item) => {
...
(item) => item.testId);
    const workflows = value.workflows.map(/** 将当前元素转换为目标投影，并保持集合顺序与一一对应关系。 */
(item) => {
      if (!record(item) || typeof item.variantId !== "string" || !Array.isArray(item.steps) || item.steps.length < 1 || item.steps.length > MAX_STEPS || item.steps.some(/** 按当前业务条件筛选或判断元素，不修改原始集合。 */
(step) => typeof step !== "string" || !step.trim())) throw new Error("workflows 格式无效");
    ...
    const outputSections = value.outputSections.map(/** 将当前元素转换为目标投影，并保持集合顺序与一一对应关系。 */
(item) => {
...
function materializeSections(raw: RawWorksheet["outputSections"][number]["sections"], units: TextUnit[], answer: string, variantId: string) {
  ...
  const sections = raw.length <= units.length ? raw : [
    ...
    { ...raw[units.length - 1]!, label: raw.slice(units.length - 1).map(/** 将当前元素转换为目标投影，并保持集合顺序与一一对应关系。 */

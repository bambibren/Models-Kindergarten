...
const runtime = new AgentRuntime(model, new ToolRegistry(sandbox));
...
function createStudent(): ModelStudent {
  ...
  if (provider !== "ollama") {
    throw new Error(
      `V1 只实现 ollama Provider；${provider} 仅保留在 ModelProvider 适配接口中`,
    );
  }
  return {
    ...
    agentConfig: {
      systemPrompt:
        ...
        "你是 Models Kindergarten 中的本地 8B ModelStudent。请使用简洁、清楚的中文回答。需要读取或写入文件时必须调用 read_file/write_file，不得猜测文件内容；缺少完成任务所需的信息时调用 ask_user。所有文件工具都只作用于隔离沙箱。",

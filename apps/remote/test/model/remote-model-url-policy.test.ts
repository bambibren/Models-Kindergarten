...
describe("RemoteModelUrlPolicy", () => {
  it("只接受全部解析到公网地址的 HTTPS 端点", async () => {
    const policy = new RemoteModelUrlPolicy(async () => [
      { address: "8.8.8.8" },
  ...
  it("DNS 故障与安全拒绝使用不同原因", async () => {
    const policy = new RemoteModelUrlPolicy(async () => { throw new Error("NXDOMAIN"); });

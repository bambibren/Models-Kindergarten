...
describe("ArtifactService", () => {
  it("按内容哈希复用 Blob，并按 operationId 幂等发布", async () => {
  ...
  it("覆盖保持 Artifact ID 和 vN，创建新版本由服务端自增", async () => {
    const fixture = await setup();
  ...
  it("拒绝跨 Session 覆盖同一 ID，但允许创建服务端新版本", async () => {
    const fixture = await setup();
  ...
  it("每个 Artifact ID 只保留包含当前内容在内的最近三份修订，并可按步数回滚", async () => {
    const fixture = await setup();
  ...
  it("禁止跨 owner 读取，并且发布源只能来自当前 Session Workspace", async () => {
    const fixture = await setup();
  ...
  it("HTML Bundle 保留 JavaScript，并通过 base 地址解析相对资源", async () => {
    const fixture = await setup();
  ...
  it("从既有 Artifact Blob 物化到新 Session，不读取来源 Workspace", async () => {
    const fixture = await setup();
    const source = await workspace(fixture.workspaces, "session-source");
  ...
  it("归档后仍可恢复和读取，Bundle 遇到符号链接时直接拒绝", async () => {
    const fixture = await setup();
    const sandbox = await workspace(fixture.workspaces, "session-a");
  ...
  it("Blob 被破坏后直接失败，不回退到 Workspace", async () => {
    const fixture = await setup();
    const sandbox = await workspace(fixture.workspaces, "session-a");
  ...
  it("图片与 PDF 预览使用 inline raw 路由，下载仍由 content 路由负责", async () => {
    const fixture = await setup();

...
describe("OnlyOfficePreviewService", () => {
  it("为当前 PPTX 版本生成短时读取票据和可选 DocumentServer JWT", () => {
  ...
  it("拒绝过期、篡改和跨 Artifact 复用的票据", () => {
    ...
    expect(() => preview.verify("artifact_another", token)).toThrow("无效或已过期");
    expect(() => preview.verify("artifact_pptx_preview", `${token.slice(0, -1)}x`)).toThrow("无效或已过期");
    now += 2_000;
    expect(() => preview.verify("artifact_pptx_preview", token)).toThrow("无效或已过期");
  });

...
import { ContentRenderer, type ArtifactNavigation } from "./ContentRenderer.js";
import { formatTokenCount } from "../tokens/token-format.js";
import { FileText } from "lucide-react";

/** 渲染「MessageEntryView」界面投影，所有业务事实仍由上层状态与服务端提供。 */
export function MessageEntryView({ entry, artifactNavigation }: { entry: MessageEntry; artifactNavigation?: ArtifactNavigation | undefined }) {
...
(mention) => <span key={mention.artifactId} title={mention.uri}><FileText size={11} />{mention.displayName}<small>{mention.artifactId.slice(-6)}</small></span>)}</div> : null}
    <div className="user-bubble"><ContentRenderer artifactNavigation={artifactNavigation} content={entry.content} /></div>
  ...
  return <article className="assistant-message">
    <div className="assistant-body">
      <ContentRenderer artifactNavigation={artifactNavigation} content={entry.content} streaming={entry.status === "streaming"} />

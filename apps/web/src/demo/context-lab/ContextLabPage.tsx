...
export function ContextLabPage() {
  ...
  const [versions, setVersions] = useState<DemoContextVersion[]>(() => mode === "history_turn" ? createHistoryVersions() : createFreshVersions());
  ...
  function removeComparison(id: DemoContextVersion["id"]) {
    ...
    setVersions(next);
    if (!next.some((version) => version.id === activeId)) setActiveId(next[0]?.id ?? "a");
    setImportedAgentNames((current) => {
  ...
  function patchModule(moduleId: ContextModuleId, updater: Parameters<typeof updateModule>[3]) {
    setVersions((current) => updateModule(current, activeId, moduleId, updater));
  }

  function importAgent(agentId: string) {
    const agent = availableAgents.find((candidate) => candidate.id === agentId);
    if (!agent) return;
    setVersions((current) => replaceVersionModules(current, activeId, agent.modules));
    setImportedAgentNames((current) => ({ ...current, [activeId]: agent.name }));
    setImportOpen(false);
  }
  ...
  return <main className="mk-demo-app mk-context-lab">
    ...
    <section className="mk-context-prompt" aria-labelledby="context-prompt-title">
      ...
      {mode === "history_turn"
        ...
        : <textarea aria-label="实验问题" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="输入一个要让不同上下文策略共同回答的问题…" rows={3} />}
    </section>
    ...
    <div className="mk-context-workbench">
      <aside className="mk-context-version-rail">
        ...
        <div className="mk-context-version-list">
          {versions.map((version) => <div className={`mk-context-version-tab ${activeId === version.id ? "active" : ""}`} key={version.id}>
            <button type="button" onClick={() => setActiveId(version.id)}>
            ...
            {!version.locked && versions.length > 2 && <button aria-label={`删除${version.name}`} className="mk-context-version-remove" type="button" onClick={() => removeComparison(version.id)}><Trash2 size={12} /></button>}
          </div>)}
        </div>
      ...
      {active && <section className="mk-context-editor">
        <header>
          ...
          <div className="mk-context-editor-actions">
            {!active.locked && <button type="button" onClick={() => setImportOpen(true)}><Download size={13} />导入已有 Agent 策略</button>}
    ...
    {importOpen && <div className="mk-context-import-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setImportOpen(false); }}>
      <section aria-labelledby="agent-import-title" aria-modal="true" className="mk-context-import-dialog" role="dialog">
        <header><div><strong id="agent-import-title">导入已有 Agent 策略</strong><small>只覆盖当前可编辑版本的策略表单。</small></div><button aria-label="关闭导入 Agent" type="button" onClick={() => setImportOpen(false)}><X size={16} /></button></header>
        <div>{availableAgents.map((agent) => <button key={agent.id} type="button" onClick={() => importAgent(agent.id)}><Bot size={15} /><span><strong>{agent.name}</strong><small>{agent.description}</small></span><em>{agent.state === "active" ? "可用" : "草稿"}</em></button>)}</div>
      </section>
    </div>}
  </main>;
}

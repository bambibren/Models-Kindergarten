...
() => {
  it("A/B 从相同且隔离的 Agent、模型和推理配置开始", /** 执行当前测试场景并断言可观察结果，不依赖其它用例的执行顺序。 */
() => {
...
(item) => item.modelStudentId === "model-a" && item.reasoningProfile === "auto")).toBe(true);
    expect(lanes[0]?.policy).toEqual(lanes[1]?.policy);
    expect(lanes[0]?.policy).not.toBe(lanes[1]?.policy);
  });

  it("每个 Test 可独立修改模型和推理档位", /** 执行当前测试场景并断言可观察结果，不依赖其它用例的执行顺序。 */
() => {
...
(item) => item.label)).toEqual(["A", "B"]);
  ...
  it("导入 Agent 只替换当前 Test 的 Agent 策略，不改模型和推理", /** 执行当前测试场景并断言可观察结果，不依赖其它用例的执行顺序。 */
() => {

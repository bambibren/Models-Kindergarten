...
type Tab = "experiments" | "agents" | "models" | "mcps" | "skills";
...
export function MePage() {
  ...
  const load = useCallback(async () => {
    const [agents, models, mcps, skills, experiments] = await Promise.all([controlApi.agents(), controlApi.models(), controlApi.mcps(), controlApi.skills(), controlApi.experiments(true)]);
    return { agents: agents.items, models: models.items, mcps, skills: skills.items, experiments };
  }, []);
  ...
  return <main className="product-page"><ProductNav active="me" /><div className="product-me-shell"><aside><div><UserRound size={23} /></div><strong>Admin</strong><span>本地管理员</span><p>本轮使用固定 local-admin；账号系统不在当前范围内。</p></aside><section><header><span>ADMIN · PERSONAL SPACE</span><h1>我的</h1></header><nav className="product-tabs">{tabs.map((item) => { const Icon = item.icon; return <button className={tab === item.id ? "active" : ""} key={item.id} type="button" onClick={() => select(item.id)}><Icon size={14} />{item.label}</button>; })}</nav>
    {state.phase === "loading" ? <LoadingState /> : state.phase === "error" ? <ErrorState {...state} retry={retry} /> : <ResourcePanel tab={tab} data={state.data} retry={retry} />}</section></div></main>;
}
...
async function loadAll() { const [agents, models, mcps, skills, experiments] = await Promise.all([controlApi.agents(), controlApi.models(), controlApi.mcps(), controlApi.skills(), controlApi.experiments(true)]); return { agents: agents.items, models: models.items, mcps, skills: skills.items, experiments }; }
function Panel({ title, action, children }: { title: string; action?: React.ReactNode; children: React.ReactNode }) { return <section className="product-panel"><header><div><strong>{title}</strong><small>服务端持久记录</small></div>{action}</header><div className="product-resource-list">{children || <div className="product-empty">暂无记录</div>}</div></section>; }
